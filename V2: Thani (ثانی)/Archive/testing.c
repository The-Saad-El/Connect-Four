...
void setPlayers()
{
    ...
    switch(1)       // PvAI()
    {
        ...
        case 1:
        {
            ...
            strcpy(game.player2Name, "Thalith"); break;
...
int checkOpponentWin(char opponentToken)
{
...
int lvl3_thalith_1win_2dontLose_3prevent3InARow_4playRandom(char myToken, char opponentToken)  //
{
    ...
    */
    ...
    // blocking opponent's instant win
    printf("\n> Checking opponent's win...");
    wait(500);
    if (checkOpponentWin(opponentToken) != -1){ return checkOpponentWin(opponentToken); }


    // preventing the opponent's potential 3-in-a-row
    int row, col;
    printf("\n> Checking opponent's potential 3-in-a-row");
    for (int i = 0; i < numOfAvailableColumns; i++)
    {
        wait(500);
        makeGameBoardCopy();
        ...
        printf("\n\t> i: [%d] | numOfAvailableColumns: [%d]  ||  Col (availableColumn[%d]): [%d] | Row: [%d]", i, numOfAvailableColumns, i, col, row);
        int filled, empty;
    ...
    int move = availableColumns[randomIndex];
    printf("\n> [Random Move] RandomIndex: [%d], move: [%d]", randomIndex, move);
    return move;
}
...
void getAIMove()
{
    ...
    animateText(AIDialogues[randomIndex], 33);
    wait(1000);
